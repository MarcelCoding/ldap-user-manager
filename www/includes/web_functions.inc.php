...
function validate_passkey_cookie()
{
    ...
    if (isset($_COOKIE['orf_cookie'])) {
        ...
        if (!$session_file) {
            ...
            $IS_ADMIN = FALSE;
            if ($SESSION_DEBUG == TRUE) {
                error_log("$log_prefix Session: orf_cookie was sent by the client but the session file wasn't found at /tmp/$filename", 0);
            }
        } else {
            ...
            <ul class="nav navbar-nav">
                ...
                foreach ($MODULES as $module => $access) {
                    ...
                    if ($VALIDATED == TRUE) {
                        ...
                        if ($IS_ADMIN == FALSE and $access == 'admin') {
                            $show_this_module = FALSE;
                        }
                    } else {
                        if ($access != 'hidden_on_login') {
...
function set_page_access($level)
{
    ...
    if ($level == "admin") {
        if ($IS_ADMIN == TRUE and $VALIDATED == TRUE) {
            ...
            if ($SESSION_DEBUG == TRUE) {
                error_log("$log_prefix Session: UNAUTHORISED: page security level is 'admin' but IS_ADMIN = '${IS_ADMIN}' and VALIDATED = '${VALIDATED}' (user) ", 0);
            }
            exit(0);
        }
    }
    ...
    if ($level == "user") {
        if ($VALIDATED == TRUE) {
            ...
            header("Location: //" . $_SERVER["HTTP_HOST"] . "/index.php?unauthorised\n\n");
            if ($SESSION_DEBUG == TRUE) {
                error_log("$log_prefix Session: UNAUTHORISED: page security level is 'user' but VALIDATED = '${VALIDATED}'", 0);
            }
            exit(0);
        }
    }
